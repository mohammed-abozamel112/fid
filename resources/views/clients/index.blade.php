<x-master-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6 {{ $flexDirection }}">
            ...
            <a href="{{ route('clients.create', ['lang' => app()->getLocale()]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 {{ $textAlign }}">
                            {{ $client->name_ar }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 {{ $textAlign }}">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $client->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $client->status ? ($isRtl ? 'نشط' : 'Active') : ($isRtl ? 'غير نشط' : 'Inactive') }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium {{ $textAlign }}">
                            <a href="{{ route('clients.edit', ['lang' => app()->getLocale(), 'client' => $client]) }}" class="text-yellow-600 hover:text-yellow-900 mr-3">
                                {{ $isRtl ? 'تعديل' : 'Edit' }}
                            </a>
                            <form action="{{ route('clients.destroy', ['lang' => app()->getLocale(), 'client' => $client]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('{{ $isRtl ? 'هل أنت متأكد؟' : 'Are you sure?' }}')">
                                    {{ $isRtl ? 'حذف' : 'Delete' }}
                                </button>
                            </form>
                        </td>
